''' reparametrizing the whh matrix by multiplying by a random gaussian matrix
...
w_mask=np.float32(np.random.normal(size=[hp.N_HIDDEN,hp.N_HIDDEN])/np.sqrt(hp.N_HIDDEN))

theta_prime=theta.copy()
theta_prime['w_hidden']=hp.G_MASK*tf.matmul(theta['w_hidden'] , w_mask)
