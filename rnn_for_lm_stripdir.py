...
hp.W_STRIP=100
np.exclude_param = ['hopf_v','hopf_mu']
hp.parse_from_command_line(sys.argv)
...
theta_prime['w_hidden']=theta['w_hidden'] * \
                        np.tril(np.ones([hp.N_HIDDEN,hp.N_HIDDEN]),hp.W_STRIP) *\
                        np.triu(np.ones([hp.N_HIDDEN,hp.N_HIDDEN]),-hp.W_STRIP)
inmask=np.zeros([hp.VOCAB_SIZE,hp.N_HIDDEN])
# outmask=np.zeros([hp.N_HIDDEN,hp.VOCAB_SIZE])
inmask[:,:hp.W_STRIP]=1
# outmask[hp.W_STRIP:,:]=1
theta_prime['w_input']=theta['w_input']*inmask
# theta_prime['w_out']=theta['w_out']*outmask
